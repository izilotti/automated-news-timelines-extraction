/** 
 * A Co-raining algorithm for CRF++, training on sentences.
 * 
...
public class CoTraining_CRF_Sentences {

	// Flags
	public static boolean DEBUG = false;				// Turn on/off debug modes		
	public static int _mode = 1;						// Group number of model combination:
														// (Mode 1: Token model + POS model
														// Mode 2: Target position model + Context model)
	
	public static boolean _hasModel1 = true;			// If includes model 1
	public static boolean _hasModel2 = true;			// If includes model 2
	public static boolean _hasCombinedModel = true;		// If includes combined models
	
	// Parameters
	public static int _windowSize = 2;					// Window Size
	public static int _iteration = 25;					// Iteration numbers
	public static int _maxSelectionLimit = 100;			// Maximum number of selected files
...
//	public static double _minSum = 0.1668;  
	...
	public static String _unlabeledDocsDir = 
		_crfFolder.concat("/unlabeled/NewsOnly_sentences"); 						// Path of unlabeled documents
	public static String _testDocsDir = _crfFolder.concat("/test");					// Path of testing data

	public static String _trainingDataDir = _crfFolder.concat("/train.data");		// Path of CRF++ training data
	public static String _unlabeledDataDir = _crfFolder.concat("/unlabel.data");	// Path of unlabeled data to CRF++
	public static String _testingDataDir = _crfFolder.concat("/test.data");			// Path of CRF++ testing data
	...
	public static ArrayList<String> _testTrueLabelList = new ArrayList<String>(); 	// List of true TimeML tags
	public static ArrayList<String> _testPredLabelList = new ArrayList<String>(); 	// List of predicted TimeML tags
	public static ArrayList<String> _crfPredLabel = new ArrayList<String>(); 		// List of predicted TimeML tags
	...
	public static ArrayList<String> _selectedFile = new ArrayList<String>();
	 ...
	 * @return <none>
	 */
	public CoTraining_CRF_Sentences() {
		// currently empty
	}
	

	/**
	 * Create a list for labeled documents
	 * 
	...
	/**
	 * Parse XML documents, assign POS tags to each token,
	 * and create unlabeled.data file for CRF++
	 * 
	 * @param data: the TimeMLParser format data 
	 * 		  docsList: a list of documents to be parsed
	 * @return <none>
	 */
	 ...
	 * @return <none>
	 */
	private void createUnlabeledData(ArrayList<String> docsList)
			throws Exception {
		...
		out.close();
		if (DEBUG == true)
			System.out.println("[CRFpp_Sentences] unlabel.data created.\n ");
	}
	 ...
	 * POS Tagging
	 * 
	 * @param nodeList: list of text nodes 
	 * 		  tokenList: list of tokens 
	 * 		  POSList: list of POS tags
	 * @return <none>
	 */
	...
	/**
	 * Label new documents using a model
	 * 
	 * @param model: the directory of model file 
	 * 		  percent: the portion of documents to be selected each time
	 * 
	...
	private ArrayList<String> selectTopResults(String model) throws Exception {
		...
		String command = _CRFExecutable + "crf_test -v1 -m " + model + " "
				+ _unlabeledDataDir;
		...
		while ((str = buffIn.readLine()) != null) {
			if (!str.isEmpty()) {
				...
				if (str.startsWith("# 0.")) {
					// Skip some operations this is the first time see result "#0.x"
					if (docCounter > 0){
						// Make a copy of predicted labels of the previous segment
					...
					// Obtain the conditional probability value of current document
					condLikelihood[docCounter] = Double.parseDouble(str.substring(2));
					...
					// If predicted label is TIMEX3, increment the TIMEX3 counter
		...
		for (int k=0; k<condLikelihood.length; k++){
			...
			double normalizedCL = Math.pow(condLikelihood[k], 1.0/(double)segLength[k]);
		...
		int selectCounter = 0;
		// Sort the HashMap of conditional probabilities by values in descending order
		for (Iterator<Integer> i = sortByValue(hmProb).iterator(); i.hasNext();) {
			...
			// _threshold_TIMEX3){
			//if (eventRatio[key] >= _threshold_EVENT
			...
			//		&& hmProb.get(key) > 0) {
				...
				if (!selectedFile.contains(_unlabeledDocsList.get(key))) {
					...
					_selectedFile.add(_unlabeledDocsList.get(key));
				}
	 ...
	 * 
	 */
	private void updateTrainData(ArrayList<String> selectedFiles, int iteration) throws IOException {
		
		// Open train.data and append new text to the end of existing text
		FileWriter fstream = new FileWriter(_trainingDataDir, true);
		BufferedWriter out = new BufferedWriter(fstream);

		for (int i=0; i<selectedFiles.size(); i++) {
			int key = _selectedKeys.get(i);
			for (int j = 0; j < _crfPredLabelList.get(key).size(); j++) {
				try{
	...
	/**
	 * Evaluate the performance of a certain model
	 * 
	 * @param crf: a CRFpp_Sentences model
	 * @return <none>
	 * @throws Exception 
	 */
	private void evaluateModel(CRFpp_sentences crf) throws Exception {
		...
		for (TimeMLParser timeMLInstance : _testingData){
			...
			_testFileCounter++;
			if (DEBUG == true)
				System.out.println("[Evaluator] Testing: " + _testingFilesList.get(_testFileCounter-1));
        	...
        	// Calculate model's performance (precision, recall, accuracy and F-score)
        	double[] pc = PerformanceCounter.countPerformance(_windowSize, _testTrueLabelList, _testPredLabelList);
		...
		double[] results = PerformanceCounter.countAveragePerformance(all_precision, all_recall, 
				all_accuracy, all_fscore, testFileSize);
	...
	public static void main(String[] args) throws Exception {
		...
		ArrayList<CRFpp_sentences> modelList = new ArrayList<CRFpp_sentences>();		// A list of models for co-training

		// Instantiate CRF models with various parameter sets
		CRFpp_sentences crf1 = null;
		CRFpp_sentences crf2 = null;
		CRFpp_sentences crf_combined = null;
		
		if (_mode == 1){	/** Token model + POS model **/
			crf1 = new CRFpp_sentences(".tml", /* Document type */
					...
					false /* If including POS tags */,
					false /* If has context only*/ );

			crf2 = new CRFpp_sentences(".tml", /* Document type */
					 _labeledDocsDir, /* Directory of training data */
					 _testDocsDir, /* Directory of testing data */
					 _trainingDataDir, /* Directory of train file for CRF++ */
					 _testingDataDir, /* Directory of test file for CRF++ */
					 ...
					 _windowSize, /* Window Size */
					 false, /* If including tokens */
					 true /* If including POS tags */,
					 false /* If has context only*/ );

			crf_combined = new CRFpp_sentences(".tml", /* Document type */
					 _labeledDocsDir, /* Directory of training data */
					 _testDocsDir, /* Directory of testing data */
					 _trainingDataDir, /* Directory of train file for CRF++ */
					 _testingDataDir, /* Directory of test file for CRF++ */
		...
		else if (_mode == 2){	/** Target position model + Context model **/
			crf1 = new CRFpp_sentences(".tml", /* Document type */
					_labeledDocsDir, /* Directory of training data */
					_testDocsDir, /* Directory of testing data */
					_trainingDataDir, /* Directory of train file for CRF++ */
		...
		CoTraining_CRF_Sentences ct = new CoTraining_CRF_Sentences();
		...
		for (int iter = 1; iter <= N; iter++) {		
			...
			for (int k=0; k<modelList.size(); k++){
				...
				for(int i=0; i<_unlabeledDocsList.size(); i++){
					
					System.out.println(_unlabeledDocsList.get(i));
				}
				System.out.println("_ulTokensList: " + _ulTokensList.size());
				System.out.println("_ulPOSsList: " + _ulPOSsList.size());
				System.out.println("_selectedKeys: " + _selectedKeys.size());
				System.out.println("_crfPredLabelList: " + _crfPredLabelList.size());
				System.out.println();
				...
				// test.data are created redundantly
				...
				// If no unlabeled file left, break loop of models
				if (_unlabeledDocsList.isEmpty()) break;
...
//				ArrayList<TimeMLParser> trainingData = new ArrayList<TimeMLParser>();
			...
			if(iter == 14) {
				DocUtil.createTextFile("data/CoTraining/CRFpp/TrainDataCopies/list.txt", 
						_selectedFile);
			}
		...
		System.out.println("The initial number of unlabeled sentences: " + _numOfUnlabeledDocs);
		System.out.println("Maximum number of selected sentences: " + _maxSelectionLimit);
